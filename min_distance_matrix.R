...
min_distance.matrix.node2node <- function(network, cppRouting_graph, from, to){
    #Reset the row numbers to can match with the output
    #cppRouting uses not the row label, uses the row number
    row.names(from) <- NULL
    row.names(to) <- NULL
    distances <- cppRouting::get_distance_matrix(
    ...
    for (id_from in seq(length(from))){
        for (id_to in seq(length(to))){
            from_ <- append(from_, from[[id_from]])
            to_ <- append(to_, to[[id_to]])
            if (is.na(distances[id_from, id_to])) {
                distance_ <- append(distance_, Inf)
            } else {
                distance_ <- append(distance_, distances[id_from, id_to])
            }
        }
    }
    ret <- data.frame(
